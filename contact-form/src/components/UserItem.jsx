import React from "react";

const UserItem = ({ userList, handleDelete }) => {
  return (
    <>
      {userList.map((data, idx) => {
        return (
          <li
            key={idx}
            className="list-none bg-zinc-100 rounded-xl px-6 py-3 w-[48%] flex items-start gap-4 shadow-sm"
          >
            <img
              className="size-12 object-cover rounded-full shadow-sm"
              src={data.profile}
              alt=""
            />
            <div className="details flex-1 flex justify-between items-end">
              <div>
                <h3 className="font-semibold text-lg text-shadow-xs">
                  {data.fullname}
                </h3>
                <h5 className="text-sm font-medium text-neutral-400">
                  {data.phone}
                </h5>
              </div>
              <svg
              onClick={()=> handleDelete(idx)}
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                stroke-width="1.5"
                stroke="currentColor"
                class="size-6"
                className="bg-red-500 size-7 cursor-pointer active:scale-95 text-white p-1 rounded-md"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
                />
              </svg>
            </div>
          </li>
        );
      })}
    </>
  );
};

export default UserItem;
